/*
 Code implements Motor Code and Hill Assit on LEDs thru PWM with Timer4, by reading Joystick values
 Setup:
 Button: PA3
...
int main()
{
	...
	while (1)
	{
		...
		button = (GPIOA->IDR & GPIO_IDR_IDR3);   //Read button status
		if(button && !(prev_button))   //Button press
		{
			...
			prev_button = (GPIOA->IDR & GPIO_IDR_IDR3);
